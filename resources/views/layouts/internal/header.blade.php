<header
    class="bg-gradient-to-r from-orange-900 via-orange-800 to-red-900 text-white shadow-lg border-b border-orange-700/30">
        ...
        <div class="flex items-center justify-between h-16 lg:h-18">
            ...
            <div class="flex items-center min-w-0">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 group">
                    <div class="relative">
                        ...
                        <div
                            class="absolute inset-0 bg-white/10 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        </div>
                    </div>
                    <div class="hidden sm:block">
                        <h1
                            class="font-bold text-lg lg:text-xl group-hover:text-orange-200 transition-colors duration-200">
                            Lapar Chicken
                        </h1>
            ...
            <div class="hidden lg:flex items-center space-x-4">
                ...
                @if (Auth::user()->getPrimaryRole()?->name == 'Super Admin')
                    ...
                    @if (isset($showBranchSelector) && $showBranchSelector && isset($branchItem) && $branchItem->count() > 0)
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" type="button"
                                ...
                                <svg class="w-4 h-4 ml-3 transition-transform duration-200"
                                    :class="open ? 'rotate-180' : ''" fill="currentColor" viewBox="0 0 24 24">
                            ...
                            <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                ...
                                {{-- Header --}}
                                <div
                                    class="px-6 py-4 bg-gradient-to-r from-orange-50 to-red-50 border-b border-gray-100">
                                    <h3 class="text-sm font-semibold text-gray-800 flex items-center">
                                        <svg class="w-4 h-4 mr-2 text-orange-600" fill="currentColor"
                                            viewBox="0 0 24 24">
                                            <path
                                                d="M19 7h-3V6a4 4 0 0 0-8 0v1H5a1 1 0 0 0-1 1v11a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3V8a1 1 0 0 0-1-1z" />
                                        </svg>
                                ...
                                <div class="max-h-80 overflow-y-auto">
                                    ...
                                    <a href="{{ route('dashboard', ['clear_dashboard_branch' => 1]) }}"
                                        ...
                                        <div class="flex items-center">
                                            <div
                                                class="w-10 h-10 bg-gradient-to-br from-orange-400 to-red-500 rounded-lg flex items-center justify-center mr-3">
                                    ...
                                    @if ($productionCenters->count() > 0)
                                        ...
                                        @foreach ($productionCenters as $item)
                                            <a href="{{ route('dashboard', ['branch_id' => $item->id]) }}"
                                                class="flex items-center justify-between px-6 py-4 hover:bg-orange-50 transition-colors duration-150 {{ isset($selectedBranch) && $selectedBranch && $selectedBranch->id == $item->id ? 'bg-orange-50 border-l-4 border-orange-500' : '' }}">
                                                <div class="flex items-center">
                                                    <div
                                                        class="w-10 h-10 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-lg flex items-center justify-center mr-3">
                                                    ...
                                                    <div>
                                                        <div class="font-semibold text-gray-800">{{ $item->name }}
                                                        </div>
                                                        <div class="text-xs text-gray-500">{{ $item->code }}</div>
                                                    </div>
                                                </div>
                                                @if (isset($selectedBranch) && $selectedBranch && $selectedBranch->id == $item->id)
                                                    <div
                                                        class="w-6 h-6 bg-green-100 rounded-full flex items-center justify-center">
                                                        <svg class="w-4 h-4 text-green-600" fill="currentColor"
                                                            viewBox="0 0 24 24">
                                    ...
                                    @if ($retailBranches->count() > 0)
                                        ...
                                        @foreach ($retailBranches as $item)
                                            <a href="{{ route('dashboard', ['branch_id' => $item->id]) }}"
                                                class="flex items-center justify-between px-6 py-4 hover:bg-orange-50 transition-colors duration-150 {{ isset($selectedBranch) && $selectedBranch && $selectedBranch->id == $item->id ? 'bg-orange-50 border-l-4 border-orange-500' : '' }}">
                                                <div class="flex items-center">
                                                    <div
                                                        class="w-10 h-10 bg-gradient-to-br from-blue-400 to-indigo-500 rounded-lg flex items-center justify-center mr-3">
                                                        <svg class="w-5 h-5 text-white" fill="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path
                                                                d="M19 7h-3V6a4 4 0 0 0-8 0v1H5a1 1 0 0 0-1 1v11a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3V8a1 1 0 0 0-1-1z" />
                    ...
                    @endif
                @else
                    {{-- Tampilan untuk User Biasa (Hanya informasi cabang mereka sendiri) --}}
                    @php
                        $userBranch = Auth::user()->branch; // Asumsi relasi branch ada di model User
                    @endphp
                    <div
                        class="flex items-center px-4 py-2.5 bg-white/10 rounded-lg backdrop-blur-sm border border-white/20">
                        <div class="flex items-center">
                            <div
                                class="w-10 h-10 bg-gradient-to-br from-blue-400 to-indigo-500 rounded-lg flex items-center justify-center mr-3">
                                <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="text-left">
                                <div class="font-semibold text-sm text-white">
                                    {{ $userBranch->name ?? 'Cabang Tidak Terdeteksi' }}</div>
                                <div class="text-xs text-orange-200">{{ $userBranch->code ?? '-' }}</div>
                            </div>
                        </div>
                    </div>
                @endif
                        ...
                        <div x-show="open" x-transition:enter="transition ease-out duration-200"
                            ...
                            <div class="py-2">
                                <div class="px-4 py-3 border-b border-gray-100">
                                    <div class="flex items-center">
                                        <div
                                            class="w-10 h-10 bg-gradient-to-br from-orange-400 to-red-500 rounded-full flex items-center justify-center mr-3">
                                            <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                                                <path
                                                    d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" />
                                            </svg>
                                        ...
                                        <div>
                                            ...
                                            <div class="text-xs text-gray-500">{{ Auth::user()->getPrimaryRole()?->name }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit"
                                        class="flex items-center px-4 py-3 text-gray-700 hover:bg-red-50 hover:text-red-600 w-full text-left">
                                        Logout
                                    </button>
                                </form>
                            </div>
                    ...
                    @if (isset($showBranchSelector) && $showBranchSelector && isset($branchItem) && $branchItem->count() > 0)
                        <div class="flex-1">
                            <button @click="branchOpen = !branchOpen"
                                ...
                                <div class="flex items-center min-w-0">
                                    <svg class="w-4 h-4 mr-2 flex-shrink-0 text-orange-200" fill="currentColor"
                                        viewBox="0 0 24 24">
                                        <path
                                            d="M19 7h-3V6a4 4 0 0 0-8 0v1H5a1 1 0 0 0-1 1v11a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3V8a1 1 0 0 0-1-1z" />
                                    </svg>
                                ...
                                <svg class="w-4 h-4 ml-2 flex-shrink-0 transition-transform duration-200"
                                    :class="branchOpen ? 'rotate-180' : ''" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M7 10l5 5 5-5z" />
                                </svg>
                            </button>

                            {{-- Mobile Branch Dropdown --}}
                            <div x-show="branchOpen" x-transition:enter="transition ease-out duration-200"
                                x-transition:enter-start="opacity-0 -translate-y-2"
                                x-transition:enter-end="opacity-100 translate-y-0"
                                x-transition:leave="transition ease-in duration-150"
                                x-transition:leave-start="opacity-100 translate-y-0"
                                x-transition:leave-end="opacity-0 -translate-y-2" @click.away="branchOpen = false"
                                ...
                                <div
                                    class="px-4 py-3 bg-gradient-to-r from-orange-50 to-red-50 border-b border-gray-100">
                                ...
                                @if ($productionCenters->count() > 0)
                                    ...
                                    @foreach ($productionCenters as $branchItem)
                                        <a href="{{ route('dashboard', ['branch_id' => $branchItem->id]) }}"
                                            ...
                                            <div
                                                class="w-8 h-8 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-lg flex items-center justify-center mr-3">
                                ...
                                @if ($retailBranches->count() > 0)
                                    ...
                                    @foreach ($retailBranches as $branchItem)
                                        <a href="{{ route('dashboard', ['branch_id' => $branchItem->id]) }}"
                                            class="flex items-center px-4 py-3 hover:bg-orange-50 {{ isset($selectedBranch) && $selectedBranch && $selectedBranch->id == $branchItem->id ? 'bg-orange-50 border-l-4 border-orange-500' : '' }}">
                                            <div
                                                class="w-8 h-8 bg-gradient-to-br from-blue-400 to-indigo-500 rounded-lg flex items-center justify-center mr-3">
                                                <svg class="w-4 h-4 text-white" fill="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path
                                                        d="M19 7h-3V6a4 4 0 0 0-8 0v1H5a1 1 0 0 0-1 1v11a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3V8a1 1 0 0 0-1-1z" />
                    ...
                    <div class="relative">
                        <button @click="userOpen = !userOpen"
                            ...
                            <svg class="w-3 h-3 ml-1 transition-transform duration-200"
                                :class="userOpen ? 'rotate-180' : ''" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M7 10l5 5 5-5z" />
                            </svg>
                        </button>
                        ...
                        <div x-show="userOpen" x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0 -translate-y-2"
                            x-transition:enter-end="opacity-100 translate-y-0" @click.away="userOpen = false"
