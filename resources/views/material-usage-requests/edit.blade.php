...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted small mb-0">Perbarui data pengajuan ini</p>
        </div>
        <div>
    ...
    <form action="{{ route('semi-finished-usage-requests.update', $materialUsageRequest) }}" method="POST" id="materialRequestForm">
        @csrf
        @method('PUT')
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" value="{{ $materialUsageRequest->request_number }}" readonly>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Cabang Peminta</label>
                                <input type="text" class="form-control" value="{{ $materialUsageRequest->requestingBranch->name }}" readonly>
                                <input type="hidden" name="requesting_branch_id" value="{{ $materialUsageRequest->requesting_branch_id }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Tanggal Permintaan</label>
                                <input type="text" class="form-control" value="{{ $materialUsageRequest->requested_date->format('d/m/Y') }}" readonly>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Tanggal Dibutuhkan <span class="text-danger">*</span></label>
                                <input type="date" class="form-control @error('required_date') is-invalid @enderror" id="required_date" name="required_date" value="{{ old('required_date', $materialUsageRequest->required_date ? $materialUsageRequest->required_date->format('Y-m-d') : date('Y-m-d')) }}" min="{{ date('Y-m-d') }}" required>
                                @error('required_date')
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('purpose') is-invalid @enderror" id="purpose" name="purpose" value="{{ old('purpose', $materialUsageRequest->purpose) }}" placeholder="Contoh: Untuk produksi 200 porsi Ayam Crispy" required>
                                @error('purpose')
                                    <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="3" placeholder="Opsional">{{ old('notes', $materialUsageRequest->notes) }}</textarea>
                                @error('notes')
                                    <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered" id="itemsTable">
                                ...
                                <tbody id="itemsTableBody">
                                    @foreach($materialUsageRequest->items as $index => $item)
                                        <tr class="item-row">
                                            <td>
                                                <select name="items[{{ $index }}][raw_material_id]" class="form-control raw-material-select" required>
                                                    ...
                                                    @foreach($semiFinishedProducts as $material)
                                                        <option value="{{ $material->id }}" data-unit-id="{{ $material->unit_id }}" {{ $item->raw_material_id == $material->id ? 'selected' : '' }}>
                                            ...
                                            <td>
                                                <input type="number" name="items[{{ $index }}][quantity]" value="{{ old('items.'.$index.'.quantity', $item->quantity) }}" class="form-control" step="1" min="1" inputmode="numeric" pattern="[0-9]*" required>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <input type="text" name="items[{{ $index }}][notes]" value="{{ old('items.'.$index.'.notes', $item->notes) }}" class="form-control">
                                            </td>
                                            <td>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered" id="outputsTable">
                                ...
                                <tbody id="outputsTableBody">
                                    @php
                                        $rows = $materialUsageRequest->outputs->count() ? $materialUsageRequest->outputs : $materialUsageRequest->targets;
                                    @endphp
                                    @foreach($rows as $tIndex => $target)
                                        <tr class="output-row">
                                            <td>
                                                <select name="outputs[{{ $tIndex }}][product_id]" class="form-control output-product-select">
                                                    <option value="">-- Pilih Produk Jadi --</option>
                                                    @foreach($finishedProducts as $product)
                                                        <option value="{{ $product->id }}" {{ (($target->product_id ?? $target->finished_product_id) == $product->id) ? 'selected' : '' }}>
                                                            {{ $product->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                <input type="hidden" name="outputs[{{ $tIndex }}][id]" value="{{ $target->id }}">
                                            </td>
                                            <td>
                                                <input type="number" name="outputs[{{ $tIndex }}][planned_quantity]" value="{{ old('outputs.'.$tIndex.'.planned_quantity', (int) $target->planned_quantity) }}" class="form-control output-qty-input" step="1" min="1" inputmode="numeric" pattern="[0-9]*">
                                            </td>
                                            <td>
                                                <input type="text" name="outputs[{{ $tIndex }}][notes]" value="{{ old('outputs.'.$tIndex.'.notes', $target->notes) }}" class="form-control" placeholder="Opsional">
                                            </td>
                                            <td>
                ...
                <div class="card shadow">
                    <div class="card-body">
                        <div class="d-grid gap-2">
                            ...
                            <a href="{{ route('semi-finished-usage-requests.show', $materialUsageRequest) }}" class="btn btn-outline-secondary">
...
<script>
    $(document).ready(function() {
        // Set initial row count to match existing items
        window.rowIndex = {{ count($materialUsageRequest->items) }};
        // Set initial output row count (prefer outputs; fallback to targets)
        window.outputIndex = {{ $materialUsageRequest->outputs->count() ? $materialUsageRequest->outputs->count() : $materialUsageRequest->targets->count() }};
    });
</script>
