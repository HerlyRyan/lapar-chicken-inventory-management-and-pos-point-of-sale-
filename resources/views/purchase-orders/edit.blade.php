...
@section('content')
    <div class="min-h-screen bg-gradient-to-br from-orange-50 via-white to-red-50 py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            {{-- Header --}}
            <x-form.header title="Purchase Order" edit="true" :name="$purchaseOrder->order_number"
                backRoute="{{ route('purchase-orders.index') }}"
                detailRoute="{{ route('purchase-orders.show', $purchaseOrder) }}" />
                ...
                <div class="p-6 sm:p-8">
                    @if ($errors->any())
                        <div class="mb-4 p-4 rounded-lg bg-red-50 border border-red-100 text-red-700">
                            <h6 class="font-semibold mb-2"><i class="bi bi-exclamation-triangle me-1"></i> Terjadi
                                kesalahan:</h6>
                    ...
                    <form id="purchase-order-form" action="{{ route('purchase-orders.update', $purchaseOrder) }}"
                        method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div id="po_number"
                                        ...
                                        {{ $purchaseOrder->order_number }}</div>
                                    ...
                                    <select name="supplier_id" id="supplier_id" required
                                        ...
                                        @foreach ($suppliers as $supplier)
                                            <option value="{{ $supplier->id }}"
                                                ...
                                                {{ $supplier->name }} @if ($supplier->code)
                                                    ({{ $supplier->code }})
                                                @endif
                                            </option>
                                        @endforeach
                                ...
                                <input type="hidden" name="order_date" id="order_date"
                                    value="{{ old('order_date') ?? ($purchaseOrder->order_date ? $purchaseOrder->order_date->format('Y-m-d') : date('Y-m-d')) }}">
                                ...
                                <div>
                                    ...
                                    <div class="flex rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 items-center">
                                        ...
                                        <input type="text" id="supplier_phone" class="flex-1 bg-transparent outline-none"
                                            readonly
                                            value="{{ old('supplier_phone') ?? ($purchaseOrder->supplier->phone ?? '') }}">
                                    </div>
                                            ...
                                            @if ($purchaseOrder->items->count() > 0)
                                                @foreach ($purchaseOrder->items as $index => $item)
                                                    <tr class="item-row align-top"
                                                        ...
                                                        <td class="p-3 text-center font-semibold row-number">
                                                            {{ $index + 1 }}</td>
                                                        <td class="p-3">
                                                            <select name="items[{{ $index }}][raw_material_id]"
                                                                class="form-select raw-material-select w-full px-3 py-2 border rounded"
                                                                required>
                                                                <option value="">-- Pilih Bahan Mentah --</option>
                                                                <option value="{{ $item->raw_material_id }}" selected>
                                                                    {{ $item->rawMaterial->name }}</option>
                                                            </select>
                                                        ...
                                                        <td class="p-3 text-center unit-name">
                                                            {{ $item->unit_name ?? '-' }}</td>
                                                        <td class="p-3">
                                                            <div class="flex items-center justify-end gap-2">
                                                                <span
                                                                    class="text-sm bg-indigo-600 text-white px-2 py-1 rounded">Rp</span>
                                                                <input type="number"
                                                        ...
                                                        <td
                                                            class="p-3 text-center item-total font-bold text-green-600 whitespace-nowrap">
                                                ...
                                                <tr class="empty-row">
                                                    <td colspan="7" class="p-8 text-center text-gray-500">
                                                        <div class="flex flex-col items-center">
                                                            ...
                                                            <p class="text-sm">Klik tombol "Tambah Bahan Mentah" untuk
                                                                menambah item</p>
                                                        </div>
                                                    </td>
                                                </tr>
