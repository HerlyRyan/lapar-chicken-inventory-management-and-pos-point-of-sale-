        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden mt-6">
                ...
                <div class="p-6 sm:p-8">
                    ...
                    <form action="{{ route('semi-finished-products.update', $semiFinishedProduct->id) }}" method="POST"
                        ...
                        <div class="mb-8">
                            <div class="flex items-center mb-6">
                                ...
                                <div class="ml-auto text-sm">
                                    @if (isset($branchForStock) && $branchForStock)
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-info text-white">{{ $branchForStock->name }}</span>
                                    @elseif(isset($selectedBranch) && $selectedBranch)
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-info text-white">{{ $selectedBranch->name }}</span>
                                    @else
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-gray-200 text-gray-700">Semua
                                            Cabang</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div class="lg:col-span-2">
                                    <label for="name" class="block text-sm font-semibold text-gray-700 mb-2">Nama Bahan
                                    ...
                                    <input type="text" name="name" id="name"
                                        ...
                                        value="{{ old('name', $semiFinishedProduct->name) }}" required
                                        placeholder="Cth: Adonan Tepung">
                                    <p class="mt-2 text-sm text-gray-600"><i class="bi bi-info-circle mr-1"></i>Nama bahan
                                        setengah jadi</p>
                                    @error('name')
                                ...
                                <div>
                                    <label for="code"
                                        class="block text-sm font-semibold text-gray-700 mb-2">Kode</label>
                                    ...
                                    <select name="category_id" id="category_id"
                                        ...
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}"
                                                {{ old('category_id', $semiFinishedProduct->category_id) == $category->id ? 'selected' : '' }}>
                                                {{ $category->name }}</option>
                                        @endforeach
                                    </select>
                                    <p class="mt-2 text-sm text-gray-600"><a href="{{ route('categories.create') }}"
                                            target="_blank" rel="noopener" class="text-orange-600 underline"><i
                                                class="bi bi-box-arrow-up-right mr-1"></i>Tambah kategori di tab baru</a>
                                    </p>
                                    @error('category_id')
                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                    ...
                                    <select name="unit_id" id="unit_id"
                                        ...
                                        @foreach ($units as $unit)
                                            <option value="{{ $unit->id }}"
                                                {{ old('unit_id', $semiFinishedProduct->unit_id) == $unit->id ? 'selected' : '' }}>
                                                {{ $unit->unit_name }}</option>
                                        @endforeach
                                    </select>
                                    <p class="mt-2 text-sm text-gray-600"><a href="{{ route('units.create') }}"
                                            target="_blank" rel="noopener" class="text-orange-600 underline"><i
                                                class="bi bi-box-arrow-up-right mr-1"></i>Tambah satuan di tab baru</a></p>
                                    @error('unit_id')
                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                @php
                                    $canEditStock =
                                        (isset($branchForStock) && $branchForStock) ||
                                ...
                                <div>
                                    <label for="stock_quantity" class="block text-sm font-semibold text-gray-700 mb-2">Stok
                                        Saat Ini @if ($canEditStock)
                                            <span
                                                class="text-sm text-info">({{ $branchForStock->name ?? $selectedBranch->name }})</span>
                                        @endif
                                    </label>
                                    <input type="number" step="0.01" name="stock_quantity" id="stock_quantity"
                                        ...
                                        value="{{ old('stock_quantity', $currentStock) }}"
                                        {{ !$canEditStock ? 'disabled' : '' }} placeholder="0">
                                    @if (!$canEditStock)
                                        <p class="mt-2 text-sm text-yellow-700"><i
                                                class="bi bi-info-circle mr-1"></i>Pilih cabang untuk edit stok.</p>
                                    @endif
                                ...
                                <div class="lg:col-span-2">
                                    <label for="production_cost"
                                        class="block text-sm font-semibold text-gray-700 mb-2">Biaya Produksi</label>
                                    <input type="number" step="0.01" name="production_cost" id="production_cost"
                                        ...
                                        value="{{ old('production_cost', $semiFinishedProduct->production_cost) }}"
                                        placeholder="0">
                                ...
                                <div>
                                    ...
                                    <div id="previewContainer"
                                        class="border rounded-xl p-4 bg-gray-50 text-center relative"
                                        style="height:140px;">
                                        @php
                                            $stored = $semiFinishedProduct->image
                                                ? Storage::disk('public')->exists($semiFinishedProduct->image)
                                                : false;
                                            $imageUrl = $stored ? Storage::url($semiFinishedProduct->image) : '';
                                        @endphp
                                        <img id="imagePreview" src="{{ $imageUrl }}" alt="Preview"
                                            class="mx-auto max-h-24 rounded {{ $imageUrl ? 'block' : 'hidden' }}">
                                        <span id="imagePreviewText"
                                            class="text-sm text-gray-500 {{ $imageUrl ? 'hidden' : 'block' }} mt-6">Preview
                                            foto</span>
                                    </div>
                                </div>
                                ...
                                <div class="lg:col-span-2">
                                    <label class="flex items-center cursor-pointer mt-2">
                                        <input type="checkbox" name="is_active" id="is_active"
                                            ...
                                            value="1"
                                            {{ old('is_active', $semiFinishedProduct->is_active) ? 'checked' : '' }}>
                                        <span class="ml-3">
                                            <span class="text-sm font-semibold text-gray-900">Status Aktif</span>
                                            <span class="block text-xs text-gray-600 mt-1"><i
                                                    class="bi bi-info-circle mr-1"></i>Nonaktifkan jika bahan tidak
                                                digunakan lagi.</span>
                                        </span>
                                    </label>
                                </div>
