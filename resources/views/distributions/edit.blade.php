...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h3 fw-bold mb-1" style="color: var(--primary-red);">
            <i class="bi bi-truck me-2"></i>Edit Distribusi
        </h1>
        <p class="text-muted mb-0">Edit data distribusi</p>
    </div>
        ...
        <div class="card border-0 shadow-lg">
            <div class="card-header text-white text-center py-3" style="background: linear-gradient(135deg, #dc2626 0%, #ea580c 50%, #eab308 100%);">
                <h5 class="mb-0 fw-bold">
                    <i class="bi bi-pencil-square me-2"></i>Form Edit Distribusi
                </h5>
            </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="distribution_number" id="distribution_number" 
                                   ...
                                   value="{{ old('distribution_number', $distribution->distribution_number) }}" required>
                            <div class="form-text">
                            ...
                            <select name="from_branch_id" id="from_branch_id" class="form-select @error('from_branch_id') is-invalid @enderror" required>
                                ...
                                @foreach($branches as $branch)
                                    <option value="{{ $branch->id }}" 
                                        {{ old('from_branch_id', $distribution->from_branch_id) == $branch->id ? 'selected' : '' }}>
                                        {{ $branch->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="to_branch_id" id="to_branch_id" class="form-select @error('to_branch_id') is-invalid @enderror" required>
                                ...
                                @foreach($branches as $branch)
                                    <option value="{{ $branch->id }}" 
                                        {{ old('to_branch_id', $distribution->to_branch_id) == $branch->id ? 'selected' : '' }}>
                                        {{ $branch->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="semi_finished_product_id" id="semi_finished_product_id" class="form-select @error('semi_finished_product_id') is-invalid @enderror" required>
                                ...
                                @foreach($semiFinishedProducts as $product)
                                    <option value="{{ $product->id }}" 
                                        {{ old('semi_finished_product_id', $distribution->semi_finished_product_id) == $product->id ? 'selected' : '' }}>
                                        {{ $product->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="quantity" id="quantity" 
                                   ...
                                   value="{{ old('quantity', $distribution->quantity) }}" required min="1" step="0.01">
                            <div class="form-text">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="date" name="distribution_date" id="distribution_date" 
                                   ...
                                   value="{{ old('distribution_date', $distribution->distribution_date) }}" required>
                            <div class="form-text">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                                ...
                                <option value="pending" {{ old('status', $distribution->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="in_transit" {{ old('status', $distribution->status) == 'in_transit' ? 'selected' : '' }}>Dalam Perjalanan</option>
                                <option value="completed" {{ old('status', $distribution->status) == 'completed' ? 'selected' : '' }}>Selesai</option>
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea name="notes" id="notes" class="form-control @error('notes') is-invalid @enderror" 
                                      rows="3" placeholder="Catatan distribusi (opsional)">{{ old('notes', $distribution->notes) }}</textarea>
                            <div class="form-text">
