...
@section('content')
    <div class="min-h-screen bg-gradient-to-br from-orange-50 via-white to-red-50 py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            {{-- Header --}}
            <x-form.header title="Produk Siap Jual" backRoute="{{ route('finished-products.index', request()->has('branch_id') ? ['branch_id' => request('branch_id')] : []) }}" />
                ...
                <div class="p-6 sm:p-8">
                    ...
                    @if($isProductionCenter)
                        ...
                        <div class="text-center py-12">
                            ...
                            <a href="{{ route('finished-products.index') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg shadow-sm hover:shadow-md">
                                <i class="bi bi-arrow-left"></i> Kembali ke Daftar Produk
                            </a>
                        </div>
                    @else
                        <form action="{{ route('finished-products.store', request()->has('branch_id') ? ['branch_id' => request('branch_id')] : []) }}" method="POST" enctype="multipart/form-data" id="finishedProductForm" onsubmit="console.log('Form submitted!');">
                            @csrf
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                                <div class="lg:col-span-8 space-y-4">
                                    <div>
                                        <label for="name" class="block text-sm font-semibold text-gray-700 mb-2">Nama Produk Siap Jual <span class="text-red-500">*</span></label>
                                ...
                                <div class="lg:col-span-4 space-y-4">
                                    <div>
                                        <label for="code" class="block text-sm font-semibold text-gray-700 mb-2">Kode Produk</label>
                                        ...
                                        <p class="mt-2 text-sm text-gray-600"><i class="bi bi-info-circle mr-1"></i>Kode akan dibuat otomatis jika tidak diisi (Format: FP-XXX-001)</p>
                                        @error('code')
                                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                ...
                                <div class="lg:col-span-6">
                                    ...
                                    <div class="mt-2 text-sm text-gray-600">
                                        <a href="{{ route('categories.create') }}" target="_blank" rel="noopener" class="text-orange-600 underline inline-flex items-center gap-1">
                                ...
                                <div class="lg:col-span-6">
                                    ...
                                    <div class="mt-2 text-sm text-gray-600">
                                        <a href="{{ route('units.create') }}" target="_blank" rel="noopener" class="text-orange-600 underline inline-flex items-center gap-1">
                                ...
                                <div class="lg:col-span-6">
                                    ...
                                    <input type="number" name="minimum_stock" id="minimum_stock" value="{{ old('minimum_stock', 0) }}" step="0.01" min="0"
                                ...
                                <div class="lg:col-span-6">
                                    <label for="price" class="block text-sm font-semibold text-gray-700 mb-2">Harga Jual <span class="text-red-500">*</span></label>
                                ...
                                <div class="lg:col-span-6">
                                    ...
                                    <p class="mt-2 text-sm text-gray-600"><i class="bi bi-info-circle mr-1"></i>Modal dasar produk untuk perhitungan kerugian pemusnahan (tidak boleh melebihi harga jual)</p>
                                    @error('base_cost')
                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="lg:col-span-6">
                                    ...
                                    <input type="number" name="stock_quantity" id="stock_quantity" value="{{ old('stock_quantity', 0) }}" step="0.01" min="0"
                                ...
                                <div class="lg:col-span-12 mt-2">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <label class="flex items-start gap-3 p-4 rounded-xl border cursor-pointer bg-white">
                                            <input class="mt-1" type="radio" name="stock_mode" id="stock_mode_all" value="all" {{ old('stock_mode', !$selectedBranch ? 'all' : 'selected') == 'all' ? 'checked' : '' }}>
                                            <div>
                                                <div class="font-semibold text-gray-800 flex items-center gap-2"><i class="bi bi-buildings text-green-500"></i> Semua Cabang Retail</div>
                                                <div class="text-sm text-gray-500 mt-1">Stok akan diinisialisasi dengan jumlah yang sama di semua cabang retail. Pengecualian: <strong>Pusat Produksi</strong> tidak akan mendapatkan inisialisasi stok.</div>
                                            </div>
                                        </label>

                                        <label class="flex items-start gap-3 p-4 rounded-xl border cursor-pointer bg-white">
                                            <input class="mt-1" type="radio" name="stock_mode" id="stock_mode_selected" value="selected" {{ old('stock_mode', $selectedBranch ? 'selected' : 'all') == 'selected' ? 'checked' : '' }}>
                                            <div>
                                                <div class="font-semibold text-gray-800 flex items-center gap-2"><i class="bi bi-building text-indigo-500"></i> Cabang Tertentu</div>
                                                <div class="text-sm text-gray-500 mt-1">Stok hanya diinisialisasi untuk cabang yang dipilih.</div>
                                            </div>
                                        </label>
                                    </div>
                                </div>
                                ...
                                {{-- Branch Selection --}}
                                <div class="lg:col-span-12" id="branch-selection" style="display: {{ old('stock_mode', $selectedBranch ? 'selected' : 'all') == 'selected' ? 'block' : 'none' }};">
                                    ...
                                    <select name="selected_branch_id" id="selected_branch_id" class="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all duration-150 @error('selected_branch_id') border-red-300 ring-2 ring-red-200 @enderror">
                                ...
                                <div class="lg:col-span-6">
                                    ...
                                    <input type="file" name="photo" id="photo" accept="image/jpeg,image/png,image/jpg,image/gif" onchange="previewImage(this)"
                                    ...
                                    <p class="mt-2 text-sm text-gray-600"><i class="bi bi-info-circle mr-1"></i>Upload foto produk (opsional). Format: JPG, PNG, GIF. Max: 2MB</p>
                                    @error('photo')
                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="lg:col-span-6">
                                    ...
                                    <div id="image-preview-container" class="h-32 rounded-lg border-2 border-dashed border-gray-200 bg-gray-50 flex items-center justify-center">
                                        ...
                                        <img id="imagePreview" src="" alt="Preview" class="hidden object-contain h-28 rounded-lg">
                                    </div>
                                </div>
                                ...
                                <div class="lg:col-span-12">
                                    <label class="flex items-center gap-3 bg-gray-50 p-4 rounded-xl">
                                        <input type="checkbox" name="is_active" id="is_active" value="1" class="w-5 h-5 text-orange-600 border-gray-300 rounded" {{ old('is_active', 1) ? 'checked' : '' }}>
                                        <div>
                                            <div class="font-semibold text-gray-900">Status Aktif</div>
                                            <div class="text-xs text-gray-600 mt-1">Centang jika produk siap jual aktif digunakan</div>
                                        </div>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="border-t border-gray-200 pt-6 mt-6 flex gap-3 justify-end">
                                <a href="{{ route('finished-products.index', request()->has('branch_id') ? ['branch_id' => request('branch_id')] : []) }}" class="inline-flex items-center gap-2 px-4 py-2 border rounded-xl bg-white hover:bg-gray-50">
                                    <i class="bi bi-x-circle"></i> Batal
                                </a>
                                <button type="submit" id="submitBtn" class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-orange-600 to-red-600 text-white rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
        ...
        if (selectedBranchEl) {
            selectedBranchEl.addEventListener('change', function() {
                ...
                if (selected && selected.type === 'production') {
                    if (stockQuantityInput) { stockQuantityInput.value = 0; stockQuantityInput.disabled = true; }
                    if (minStockInput) { minStockInput.value = 0; minStockInput.disabled = true; }
                } else {
