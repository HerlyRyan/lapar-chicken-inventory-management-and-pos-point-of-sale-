        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
                ...
                <div class="p-6 sm:p-8">
                    <form
                        action="{{ route('finished-products.update', array_merge([$finishedProduct], request()->has('branch_id') ? ['branch_id' => request('branch_id')] : [])) }}"
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div class="lg:col-span-2">
                                    ...
                                    <input type="text" name="name" id="name"
                                        ...
                                        placeholder="Cth: Paket Ayam Geprek">
                                    @error('name')
                                ...
                                <div>
                                    ...
                                    <select name="category_id" id="category_id"
                                        ...
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}"
                                                {{ old('category_id', $finishedProduct->category_id) == $category->id ? 'selected' : '' }}>
                                                {{ $category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <p class="mt-2 text-sm text-gray-600"><i class="bi bi-info-circle mr-1"></i>
                                        <a href="{{ route('categories.create') }}" target="_blank" rel="noopener"
                                            class="underline">Tambah kategori di tab baru</a>
                                    </p>
                                    @error('category_id')
                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <select name="unit_id" id="unit_id"
                                        ...
                                        @foreach ($units as $unit)
                                            <option value="{{ $unit->id }}"
                                                {{ old('unit_id', $finishedProduct->unit_id) == $unit->id ? 'selected' : '' }}>
                                                {{ $unit->unit_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <p class="mt-2 text-sm text-gray-600"><i class="bi bi-info-circle mr-1"></i>
                                        <a href="{{ route('units.create') }}" target="_blank" rel="noopener"
                                            class="underline">Tambah satuan di tab baru</a>
                                    </p>
                                    @error('unit_id')
                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.01" name="minimum_stock" id="minimum_stock"
                                        ...
                                        value="{{ old('minimum_stock', $finishedProduct->minimum_stock) }}"
                                        placeholder="0">
                                    @error('minimum_stock')
                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.01" name="stock_quantity" id="stock_quantity"
                                        ...
                                        {{ !$branchForStock ? 'disabled' : '' }}>
                                    @if (!$branchForStock)
                                        <p class="mt-2 text-sm text-yellow-600"><i class="bi bi-info-circle mr-1"></i>Pilih
                                            cabang untuk edit stok.</p>
                                    @endif
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.01" name="base_cost" id="base_cost"
                                        ...
                                        value="{{ old('base_cost', $finishedProduct->production_cost) }}" placeholder="0"
                                        max="{{ old('price', $finishedProduct->price) }}">
                                    <p class="mt-2 text-sm text-gray-600"><i class="bi bi-info-circle mr-1"></i>Modal
                                        dasar tidak boleh melebihi harga jual</p>
                                    @error('base_cost')
                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <div id="previewContainer" class="border rounded-xl p-3 bg-gray-50 text-center"
                                        style="height: 140px; position: relative; overflow: hidden;">
                                        <img id="imagePreview"
                                            src="{{ $finishedProduct->photo ? asset('storage/' . $finishedProduct->photo) : '' }}"
                                            alt="Preview" class="mx-auto rounded"
                                            style="max-height: 110px; max-width: 100%; display: {{ $finishedProduct->photo ? 'block' : 'none' }};">
                                        <span id="imagePreviewText" class="text-sm text-gray-500"
                                            style="display: {{ $finishedProduct->photo ? 'none' : 'block' }};">Preview
                                            foto</span>
                                    </div>
                                </div>
                                ...
                                <div class="lg:col-span-2">
                                    <label class="flex items-center cursor-pointer">
                                        <input type="checkbox" name="is_active" id="is_active"
                                            ...
                                            value="1"
                                            {{ old('is_active', $finishedProduct->is_active) ? 'checked' : '' }}>
                                        <span class="ml-3">
                                            <span class="text-sm font-semibold text-gray-900">Produk Aktif</span>
                                            <span class="block text-xs text-gray-600 mt-1"><i
                                                    class="bi bi-info-circle mr-1"></i>Nonaktifkan jika produk tidak dijual
                                                lagi</span>
                                        </span>
                                    </label>
                                </div>
