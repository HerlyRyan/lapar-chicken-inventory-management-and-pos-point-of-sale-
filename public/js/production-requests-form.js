(function(){
  ...
  const fmt = (n) => {
    const v = Number(n) || 0;
    return 'Rp ' + new Intl.NumberFormat('id-ID',{minimumFractionDigits:0, maximumFractionDigits:0}).format(Math.round(v));
  };
  ...
  function updateRemoveButtonStates() {
    ...
    rows.forEach(row => {
      const removeBtn = row.querySelector('button[onclick*="removeMaterialRow"]');
  ...
  function addMaterialRow(){
    ...
    newRow.querySelectorAll('select, input').forEach(el => {
      if (el.name) el.name = el.name.replace(/\[\d+\]/, `[${materialRowIndex}]`);
    ...
    if (unitInfo){ unitInfo.id = `unit-info-${materialRowIndex}`; unitInfo.textContent=''; }
    const warning = newRow.querySelector('[id*="stock-warning"]');
    if (warning){ warning.id = `stock-warning-${materialRowIndex}`; warning.textContent=''; warning.classList.add('d-none'); }
    const total = newRow.querySelector('[id*="total-"]') || newRow.querySelector('[id*="total"]');
    if (total){ total.id = `total-${materialRowIndex}`; total.value=''; }

    // Rewire per-row handlers (keep onclick API)
    const materialSelect = newRow.querySelector('.material-select');
    if (materialSelect) materialSelect.setAttribute('onchange', `updateMaterialInfo(${materialRowIndex})`);
    const qtyInput = newRow.querySelector('.quantity-input');
    if (qtyInput) qtyInput.setAttribute('onchange', `calculateRowTotal(${materialRowIndex})`);
    const costInput = newRow.querySelector('.cost-input');
    if (costInput) costInput.setAttribute('onchange', `calculateRowTotal(${materialRowIndex})`);
    ...
    const removeBtn = newRow.querySelector('button[onclick*="removeMaterialRow"]');
    if (removeBtn){ removeBtn.setAttribute('onclick', `removeMaterialRow(${materialRowIndex})`); removeBtn.disabled = false; }
  ...
  function removeMaterialRow(index){
    const row = document.querySelector(`.material-row[data-index="${index}"]`) || document.querySelector(`[data-index="${index}"]`);
    if (row) row.remove();
    calculateGrandTotal();
    updateRemoveButtonStates();
  }
  ...
  function updateMaterialInfo(index){
    ...
    const quantityInput = document.querySelector(`input[name="items[${index}][requested_quantity]"]`);
    ...
    if (selectedOption && selectedOption.value){
      ...
      if (unitInfo) unitInfo.textContent = `Unit: ${unit} (Stok: ${Math.round(isNaN(s)?0:s).toLocaleString('id-ID')})`;
      if (costInput) {
      ...
      if (quantityInput){
        ...
        if (!isNaN(s)){
          ...
          const current = Math.round(parseFloat(quantityInput.value || '')) || 0;
          if (!isNaN(current) && current > s){
            quantityInput.value = String(sInt);
            if (warningEl){ warningEl.textContent = `Jumlah melebihi stok. Maksimum ${sInt.toLocaleString('id-ID')}`; warningEl.classList.remove('d-none'); }
          } else if (s <= 0){
            quantityInput.value = '';
            if (warningEl){ warningEl.textContent = 'Stok tidak memadai'; warningEl.classList.remove('d-none'); }
          } else {
  ...
  function calculateRowTotal(index){
    ...
    if (!quantityInput || !costInput || !totalDisplay) { calculateGrandTotal(); return; }
    ...
    const maxAttr = maxAttrRaw === '' ? NaN : Math.round(parseFloat(maxAttrRaw));
    if (!isNaN(maxAttr) && quantity > maxAttr){
      quantity = maxAttr;
      quantityInput.value = String(maxAttr);
      if (warningEl){ warningEl.textContent = `Jumlah melebihi stok. Maksimum ${maxAttr.toLocaleString('id-ID')}`; warningEl.classList.remove('d-none'); }
  ...
  function calculateGrandTotal(){
    ...
    document.querySelectorAll('.material-row').forEach(row => {
      const q = Math.round(parseFloat(row.querySelector('input[name*="requested_quantity"]')?.value || 0));
      const c = Math.round(parseFloat(row.querySelector('input[name*="unit_cost"]')?.value || 0));
      grand += q * c;
    });
  ...
  function addOutputRow(){
    ...
    if (productSelect) {
      productSelect.setAttribute('onchange', `updateOutputUnit(${outputRowIndex})`);
    }

    const removeBtn = newRow.querySelector('button[onclick*="removeOutputRow"]');
    if (removeBtn){ removeBtn.setAttribute('onclick', `removeOutputRow(${outputRowIndex})`); removeBtn.disabled = false; }

    // Enforce integer-only attributes for planned quantity in outputs and sanitize
    const plannedQty = newRow.querySelector('input[name*="[planned_quantity]"]');
    if (plannedQty) {
  ...
  function updateOutputUnit(index){
    const select = document.querySelector(`select[name="outputs[${index}][semi_finished_product_id]"]`);
    ...
    if (opt && opt.value){
      ...
      const text = unitAbbr ? `Satuan: ${unitName} (${unitAbbr})` : (unitName ? `Satuan: ${unitName}` : '');
  ...
  function attachFormValidation(){
    ...
    form.addEventListener('submit', function(e){
      ...
      outputRows.forEach(row => {
        const productSelect = row.querySelector('select[name*="[semi_finished_product_id]"]');
        ...
        if (!productVal && !qtyVal){
          row.querySelectorAll('select, input').forEach(el => { if (el.name) el.name = ''; });
        } else {
      ...
      if (!hasNonEmptyOutput){
        ...
        if (outputsContainer){
          outputsContainer.querySelectorAll('select[name^="outputs"], input[name^="outputs"]').forEach(el => { el.name = ''; });
        }
      }
    });
  }
  ...
  function init(){
    ...
    document.querySelectorAll('.material-row').forEach(row => {
      ...
      if (idx !== null) {
        try { updateMaterialInfo(idx); } catch (e) {}
      }
    });
    ...
    document.querySelectorAll('.output-row').forEach(row => {
      const idx = row.getAttribute('data-index');
      if (idx !== null) {
        try { updateOutputUnit(idx); } catch (e) {}
        // Ensure planned quantity inputs are integer-only
        const plannedQty = row.querySelector('input[name*="[planned_quantity]"]');
        if (plannedQty) {
          plannedQty.setAttribute('step', '1');
          plannedQty.setAttribute('min', '0');
          plannedQty.setAttribute('inputmode', 'numeric');
