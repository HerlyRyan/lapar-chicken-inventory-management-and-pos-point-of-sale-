(function() {
  function setupAdditionalCosts(prefix) {
    const addBtn = document.querySelector(`.js-add-cost-row[data-prefix="${prefix}"]`);
    const container = document.getElementById(`${prefix}-additional-costs-container`);
    const template = document.getElementById(`${prefix}-additional-cost-row-template`);
    ...
    if (addBtn) {
      addBtn.addEventListener('click', () => {
        try {
          ...
          try { window._prManager && window._prManager.updateTotals(prefix); } catch (_) {}
        } catch (e) {
    ...
    container.addEventListener('click', (e) => {
      ...
      if (container && container.children.length === 0) {
        container.innerHTML = '<p class="text-muted mb-0">Belum ada biaya tambahan. Klik "Tambah Biaya".</p>';
        indexCounter = 0;
      }
  ...
  class PurchaseReceiptManager {
    ...
    updateRowUI(row) {
      ...
      const ordered = Math.round(parseFloat(input.getAttribute('max')) || parseFloat(input.getAttribute('data-ordered')) || 0);
      ...
      const rejEl = row.querySelector('.js-rejected') || row.querySelector('.form-text.text-muted');
      if (rejEl) rejEl.textContent = `Otomatis: ${this.formatQty(rejected)} ${unit}`;
      // Set hidden rejected
      ...
      if (statusEl) {
        ...
        statusEl.classList.add(status === 'Diterima' ? 'text-success' : status === 'Ditolak' ? 'text-danger' : 'text-warning');
      }
      // Set hidden item_status (backend key)
      const toKey = (txt) => txt === 'Diterima' ? 'accepted' : txt === 'Ditolak' ? 'rejected' : 'partial';
      ...
      if (progEl) {
        ...
        progEl.classList.add(status === 'Diterima' ? 'bg-success' : status === 'Ditolak' ? 'bg-danger' : 'bg-warning');
      }
      if (progText) {
        const pct = ordered > 0 ? Math.min(100, Math.round((received / ordered) * 100)) : 0;
    ...
    attachRowHandlers(row) {
      ...
      const onChange = () => {
        ...
        this.updateOverallStatus(row.closest('.card-body, #qr-items-container, #items-container, form'));
        // Update totals
        try {
    ...
    initializeItemRows(container) {
      if (!container) return;
      container.querySelectorAll('.pr-row').forEach(row => this.attachRowHandlers(row));
    }
    ...
    updateOverallStatus(scopeEl) {
      ...
      const overall = allAccepted ? 'Diterima' : allRejected ? 'Ditolak' : 'Sebagian';
      ...
      ['qr-status-auto', 'pr-status-auto'].forEach(id => {
        ...
        if (el) {
          ...
          el.classList.add(overall === 'Diterima' ? 'text-success' : overall === 'Ditolak' ? 'text-danger' : 'text-warning');
        }
      });
      ...
      try {
        ...
        const form = scopeEl.closest('form') || document.querySelector('form[action*="purchase-receipts"]');
        const statusHidden = form ? form.querySelector('input[name="status"]') : null;
    ...
    hookFormValidation(form) {
      ...
      try {
        ...
        if (dateInp) {
          ...
          dateInp.addEventListener('input', function() { this.setCustomValidity(''); });
        }
        ...
        if (photoInp) {
          ...
          photoInp.addEventListener('input', function() { this.setCustomValidity(''); });
        }
      } catch (_) {}

      form.addEventListener('submit', (e) => {
        const container = form.querySelector('#qr-items-container, #items-container') || form;
        ...
        if (!rows || rows.length === 0) {
          ...
          try {
            if (window.Swal) {
              Swal.fire({ icon: 'error', title: 'Validasi Gagal', text: 'Tidak ada item untuk diterima. Pilih pesanan terlebih dahulu.' });
            } else {
        ...
        if (photoInp) {
          ...
          if (isRequired && !hasFile) {
            ...
            try {
              if (window.Swal) {
                Swal.fire({ icon: 'error', title: 'Validasi Gagal', text: 'Foto bukti penerimaan wajib diunggah.' });
              } else { alert('Foto bukti penerimaan wajib diunggah.'); }
            } catch (_) {}
            photoInp.focus();
            return;
          }
          if (hasFile) {
            ...
            if (allowed.indexOf(file.type) === -1) {
              ...
              try {
                if (window.Swal) {
                  Swal.fire({ icon: 'error', title: 'Validasi Gagal', text: 'Format foto tidak valid. Gunakan JPG atau PNG.' });
                } else { alert('Format foto tidak valid. Gunakan JPG atau PNG.'); }
              } catch (_) {}
              photoInp.value = '';
              photoInp.focus();
              return;
            ...
            if (file.size > maxSize) {
              ...
              try {
                if (window.Swal) {
                  Swal.fire({ icon: 'error', title: 'Validasi Gagal', text: 'Ukuran foto melebihi 2MB.' });
                } else { alert('Ukuran foto melebihi 2MB.'); }
              } catch (_) {}
              photoInp.value = '';
              photoInp.focus();
              return;
        ...
        try {
          const condPhotos = form.querySelectorAll('input[type="file"][name*="[condition_photo]"]');
          ...
          if (missingRequired) {
            ...
            try {
              if (window.Swal) {
                Swal.fire({ icon: 'error', title: 'Validasi Gagal', text: 'Foto kondisi item wajib diunggah untuk setiap item yang belum memiliki foto.' });
              } else { alert('Foto kondisi item wajib diunggah untuk setiap item yang belum memiliki foto.'); }
            } catch (_) {}
          ...
          condPhotos.forEach((inp) => {
            ...
            if (files && files.length > 0) {
              ...
              if (allowed.indexOf(file.type) === -1) {
                ...
                try {
                  if (window.Swal) {
                    Swal.fire({ icon: 'error', title: 'Validasi Gagal', text: 'Format foto kondisi item tidak valid. Gunakan JPG atau PNG.' });
                  } else { alert('Format foto kondisi item tidak valid. Gunakan JPG atau PNG.'); }
                } catch (_) {}
                inp.value = '';
              ...
              if (file.size > maxSize) {
                ...
                try {
                  if (window.Swal) {
                    Swal.fire({ icon: 'error', title: 'Validasi Gagal', text: 'Ukuran foto kondisi item melebihi 2MB.' });
                  } else { alert('Ukuran foto kondisi item melebihi 2MB.'); }
                } catch (_) {}
                inp.value = '';
        ...
        rows.forEach(row => {
          this.updateRowUI(row);
          const input = row.querySelector('.received-input');
          if (input && input.classList.contains('is-invalid') && !firstInvalid) firstInvalid = input;
        });
        if (firstInvalid) {
          e.preventDefault();
          e.stopPropagation();
    ...
    bindQuickReceiveModalEvents(modalEl) {
      modalEl.addEventListener('hidden.bs.modal', () => {
        ...
        if (itemsContainer) itemsContainer.innerHTML = '<p class="text-muted mb-0">Pilih PO untuk memuat item...</p>';
    ...
    resetQRAdditionalCosts() {
      const container = document.getElementById('qr-additional-costs-container');
      if (container) {
        container.innerHTML = '<p class="text-muted mb-0">Belum ada biaya tambahan. Klik "Tambah Biaya".</p>';
      }
    }
    ...
    openQuickReceive(btn) {
      try {
        ...
        const itemsContainer = document.getElementById('qr-items-container');
        if (itemsContainer) itemsContainer.innerHTML = '<div class="text-center py-3 text-muted"><div class="spinner-border spinner-border-sm me-2" role="status"></div>Memuat item...</div>';
        ...
        this.fetchItemsByPurchaseOrder(poId)
          ...
          .catch(err => {
            ...
            if (itemsContainer) itemsContainer.innerHTML = '<p class="text-danger text-center py-3">Gagal memuat item pesanan</p>';
          });
      } catch (e) {
        console.error(e);
      }
    ...
    fetchItemsByPurchaseOrder(orderId) {
      return fetch(`/api/purchase-receipts/items-by-purchase-order?order_id=${orderId}`)
        .then(resp => resp.json());
    }
    ...
    renderQuickReceiveItems(itemsContainer, data) {
      ...
      if (data.items && data.items.length) {
        data.items.forEach((item, index) => {
          const unitLabel = (item.raw_material && item.raw_material.unit && item.raw_material.unit.name)
            ? item.raw_material.unit.name
            : (item.unit_name || '');
          const orderedQty = Math.round(typeof item.quantity === 'number' ? item.quantity : parseFloat(item.quantity || 0));
          const price = Math.round(typeof item.unit_price === 'number' ? item.unit_price : parseFloat(item.unit_price || 0));
          html += `
            <div class="row mb-3 p-3 border rounded pr-row" data-price="${price}">
              ...
              <div class="col-md-2">
                ...
                <input type="text" class="form-control" value="${orderedQty} ${unitLabel}" readonly>
              </div>
              <div class="col-md-3">
                ...
                <div class="input-group">
                  ...
                  <input type="number" name="items[${index}][received_quantity]" class="form-control received-input" step="1" min="0" max="${orderedQty}" data-ordered="${orderedQty}" data-unit="${unitLabel}" value="${orderedQty}" required>
              ...
              <div class="col-12 mt-2">
                ...
                <textarea name="items[${index}][notes]" class="form-control" rows="2" placeholder="Catatan untuk item ini..."></textarea>
              </div>
            </div>`;
        });
      } else {
        html = '<p class="text-muted text-center py-3">Tidak ada item ditemukan</p>';
      }
      if (itemsContainer) {
    ...
    setupCreateView() {
      ...
      const statusField = document.querySelector('.col-md-6 select[name="status"]');
      if (statusField) {
      ...
      const renderItems = (orderId) => {
        if (!orderId) {
          itemsContainer.innerHTML = '<p class="text-muted text-center py-3">Pilih pesanan untuk menampilkan item</p>';
          return;
        }
        this.fetchItemsByPurchaseOrder(orderId)
          ...
          .catch(error => {
            ...
            itemsContainer.innerHTML = '<p class="text-danger text-center py-3">Gagal memuat item pesanan</p>';
          });
      };
      ...
      if (preselectId) {
        const option = Array.from(purchaseOrderSelect.options).find(opt => opt.value === preselectId);
        if (option) {
      ...
      if (data.items && data.items.length > 0) {
        data.items.forEach((item, index) => {
          const unitLabel = (item.raw_material && item.raw_material.unit && item.raw_material.unit.name)
            ? item.raw_material.unit.name
            : (item.unit_name || '');
          const qty = Math.round(typeof item.quantity === 'number' ? item.quantity : parseFloat(item.quantity || 0));
          const price = Math.round(typeof item.unit_price === 'number' ? item.unit_price : parseFloat(item.unit_price || 0));
          html += `
            <div class="row mb-3 p-3 border rounded pr-row" data-price="${price}">
              <input type="hidden" name="items[${index}][purchase_order_item_id]" value="${item.id}">
    ...
    setupEditView() {
      ...
      const form = document.querySelector('form[action*="purchase-receipts"][method="post"]') || document.querySelector('form');
      if (!form) return;
      const rows = form.querySelectorAll('.card-body .row.mb-3.p-3.border.rounded');
      ...
      rows.forEach(row => {
        ...
        if (maxAttr) rec.setAttribute('max', String(Math.round(parseFloat(maxAttr) || 0)));
      ...
      try { this.updateTotals('pr'); } catch (_) {}
    }

    // Compute and render totals for quick receive (qr) or create (pr)
    updateTotals(prefix) {
      const itemsContainer = document.getElementById(prefix === 'qr' ? 'qr-items-container' : 'items-container');
      if (!itemsContainer) return;
      let subtotal = 0;
      itemsContainer.querySelectorAll('.pr-row').forEach(row => {
        const price = Math.round(parseFloat(row.getAttribute('data-price')) || 0);
        const qty = Math.round(parseFloat(row.querySelector('.received-input')?.value) || 0);
      ...
      let addTotal = 0;
      const costContainer = document.getElementById(`${prefix}-additional-costs-container`);
      if (costContainer) {
        costContainer.querySelectorAll('input[name^="additional_costs"][name$="[amount]"]').forEach(inp => {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const createForm = document.querySelector('form[action*="purchase-receipts"][enctype*="multipart/form-data"]');
      ...
      ['discount_amount', 'tax_amount'].forEach((name) => {
        document.querySelectorAll(`input[name="${name}"]`).forEach((inp) => {
          ['input','change','blur'].forEach(ev => inp.addEventListener(ev, () => {
            try { window._prManager && window._prManager.updateTotals('qr'); } catch (_) {}
            try { window._prManager && window._prManager.updateTotals('pr'); } catch (_) {}
          }));
        });
      });
    } catch (_) {}
