...
if (!window.jQuery) {
  try {
    console.error(
      "[semi-finished-usage-requests-form] jQuery not found. Ensure it is loaded before this script."
    );
  } catch (_) {}
}
...
$(document).ready(function () {
  ...
  let rowIndex =
    typeof window.rowIndex === "number"
      ? window.rowIndex
      : $(".item-row").length || 0;
  try {
    console.debug(
      "[semi-finished-usage-requests-form] initial rowIndex =",
      rowIndex
    );
  } catch (_) {}
  // Keep track of output rows index (optional section). Prefer outputs; fallback to targets (legacy).
  let outputIndex =
    typeof window.outputIndex === "number"
      ? window.outputIndex
      : $(".output-row").length ||
        $("#outputsTableBody .output-row").length ||
        $(".target-row").length ||
        0;
  try {
    console.debug(
      "[semi-finished-usage-requests-form] initial outputIndex =",
      outputIndex
    );
  } catch (_) {}
  // Backward-compat: mirror to targetIndex for any legacy references
  let targetIndex =
    typeof window.targetIndex === "number" ? window.targetIndex : outputIndex;

  // Add initial row if no items exist
  if (rowIndex === 0) {
    try {
      console.debug(
        "[semi-finished-usage-requests-form] no rows found, auto-adding first row"
      );
    } catch (_) {}
    addItemRow();
  }
  ...
  function addOutputRow() {
    ...
    if (!tmplEl) {
      ...
      if (!tmplEl) {
        try {
          console.error(
            "[semi-finished-usage-requests-form] #outputRowTemplate/#targetRowTemplate not found"
          );
        } catch (_) {}
        return;
      }
    ...
    if ($tbody.length === 0) {
      try {
        console.error(
          "[semi-finished-usage-requests-form] #outputsTableBody/#targetsTableBody not found"
        );
      } catch (_) {}
      return;
    }
    ...
    try {
      console.debug(
        "[semi-finished-usage-requests-form] added output row index",
        outputIndex,
        "legacyTemplate=",
        usingLegacyTemplate
      );
    ...
    try {
      const $lastRow = $tbody.is("#outputsTableBody")
        ? $("#outputsTableBody .output-row").last()
        : $("#targetsTableBody .target-row").last();
      updateOutputUnitForRow($lastRow);
    } catch (_) {}
    outputIndex++;
    targetIndex = outputIndex; // keep in sync for legacy
    try {
      window.outputIndex = outputIndex;
  ...
  $(document).on(
    "change",
    ".output-product-select, .target-product-select",
    function () {
  ...
  $("#materialRequestForm").on("submit", function (e) {
    ...
    if (hasDuplicates) {
      ...
      showAlert(
        "warning",
        "Terdapat bahan yang duplikat. Mohon gabungkan menjadi satu item."
      );
      return false;
    }
    ...
    const $rowsForCleanup = $outputsBody.length
      ? $outputsBody.find(".output-row")
      : $targetsBody.find(".target-row");
    $rowsForCleanup.each(function () {
      const $row = $(this);
      const productId = (
        $row.find(".output-product-select, .target-product-select").val() || ""
      ).toString();
      const qty = (
        $row.find(".output-qty-input, .target-qty-input").val() || ""
      ).toString();
      const notes = (
    ...
    const $outputRows = $outputsBody.length
      ? $outputsBody.find(".output-row")
      : $targetsBody.find(".target-row");
    if ($outputRows.length > 0) {
      ...
      if (!outputsValid) {
        ...
        showAlert(
          "danger",
          "Mohon lengkapi semua baris Output (produk jadi) yang diisi."
        );
        return false;
      }
      ...
      $outputRows.each(function () {
        const pid = $(this)
          .find(".output-product-select, .target-product-select")
          .val();
      ...
      if (fpDup) {
        ...
        showAlert(
          "warning",
          "Terdapat Output dengan produk jadi yang duplikat. Mohon gabungkan menjadi satu baris."
        );
        return false;
      }
    }
  ...
  function addItemRow() {
    ...
    if (!tmplEl) {
      try {
        console.error(
          "[semi-finished-usage-requests-form] #itemRowTemplate not found"
        );
      } catch (_) {}
      return;
    }
    ...
    if (!tbody) {
      try {
        console.error(
          "[semi-finished-usage-requests-form] #itemsTableBody not found"
        );
      } catch (_) {}
      return;
    }
    ...
    try {
      console.debug(
        "[semi-finished-usage-requests-form] added row index",
        rowIndex
      );
    } catch (_) {}
    rowIndex++;
  ...
  try {
    if ($("#outputsTableBody").length) {
      $("#outputsTableBody .output-row").each(function () {
        updateOutputUnitForRow($(this));
      });
    } else {
      $("#targetsTableBody .target-row").each(function () {
        updateOutputUnitForRow($(this));
      });
    }
  } catch (_) {}
